...
describe User do
	...
	USER_ATTRS = {:name => "Fja5kd2l6s", :email => "Fja5kd2l6s@gmail.com", :password => "Fja5kd2l6s"}

  USER_ATTRS.each_key do |key|
    it "is invalid without a #{key.to_s}" do
	  	User.new(USER_ATTRS.except(key)).should_not be_valid
  	end
  end
  ...
  describe "uniqueness validations" do
    ...
    [:name, :email].each do |attribute|
      it "is invalid without a unique #{attribute}" do
        @nonunique_user[attribute] = USER_ATTRS[attribute]
